...
export async function buildClient(slug, { outDir = DIST_DIR, quiet = false } = {}) {
  ...
  // A client can carry its own clients/<slug>/styles.css, appended last so it
  // wins over the template without editing it — the template is shared with
  // every other client on it, most of which have already been approved.
  const clientCssPath = join(ROOT, 'clients', slug, 'styles.css');
  const clientCss = existsSync(clientCssPath) ? await readFile(clientCssPath, 'utf8') : '';
  ...
  // not broken.
  const subpages = [...(prepared.servicePages ?? []), ...(prepared.suburbPages ?? []), ...(prepared.sitePages ?? [])];
  const subpageFiles = [];
