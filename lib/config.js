...
export function normalise(cfg, { dir } = {}) {
  ...
  // The demo banner's CTA defaults to #enquire (presets/_base.json), which is
  // the trust template's enquiry section id — but the quote template's own
  // section is #quote. Nobody had set an explicit bannerCtaUrl, so this was a
  // dead link on every quote-template demo's "Book a 10-min walkthrough"
  // button. Only correct the untouched default; leave any explicit override.
  if (out.template === 'quote' && out.demo?.bannerCtaUrl === '#enquire') {
    out.demo = { ...out.demo, bannerCtaUrl: '#quote' };
  }
...
/* ------------------------------------------------------- the reviews we pulled */

export const REVIEWS_FILE = join(ROOT, 'data', 'google_reviews_top25.json');

/**
 * The Google reviews pulled off Maps, keyed by the same slug QUEUE.csv uses.
 *
 * This is the only source of truth for a rating, a review count or a review's
 * wording. Nothing else may supply one. Returns an empty object when the file is
 * not there, so a client that predates it is simply uncorroborated rather than
 * broken.
 */
export async function loadProspectReviews() {
  return (await readJson(REVIEWS_FILE)) ?? {};
}

/**
 * What the pull says about one client, or null.
 *
 * `rating` is null for a business Google shows no rating for, which is a fact
 * about the business, not a missing value.
 */
export async function googleRecord(slug) {
  const all = await loadProspectReviews();
...
export function findChecks(value, path = '', found = []) {
  ...
  for (const [key, child] of Object.entries(value)) {
    if (key === '_check' || key === 'dir' || key === '__runtime' || key === '_source' || key === 'palette') continue;
