...
/**
 * Australian 13, 1300 and 1800 numbers are dialled in national form only.
 * They have no +61 equivalent — they route inside Australia and cannot be
 * reached from overseas — so tel:+611300000412 is a number a phone may simply
 * refuse. Sunset Pools publish only a 1300, so its every call button pointed
 * at that. Geographic numbers and mobiles keep E.164, which dials everywhere.
 */
export const telHref = (phone) => {
  const e164 = phone?.e164 || toE164(phone?.display ?? phone);
  if (!e164) return '';
  const national = /^\+61(13\d{4}|1[38]00\d{6})$/.exec(e164);
  return `tel:${national ? national[1] : e164}`;
};
 ...
 */
export const sampleTag = (photo, where = '') =>
  photo?.sample ? html`<span class="fl-sample${raw(where === 'bottom' ? ' fl-sample-b' : '')}">Sample photo</span>` : '';
