...
export function enquiryForm(cfg) {
  const f = cfg.form ?? {};
  const errs = f.errors ?? {};
  ...
  return html`
<form class="form" id="quoteForm" novalidate data-reveal>
  <div class="row2">
  ...
  <div class="row2">
    <label for="qName">Your name <span class="req" aria-hidden="true">*</span>
      <input id="qName" name="name" type="text" autocomplete="name" required aria-describedby="qNameErr">
      <small class="field-err" id="qNameErr" hidden>${errs.name ?? 'Please add your name'}</small>
    </label>
    <label for="qPhone">Mobile <span class="req" aria-hidden="true">*</span>
      <input id="qPhone" name="phone" type="tel" autocomplete="tel" inputmode="tel" required aria-describedby="qPhoneErr">
      <small class="field-err" id="qPhoneErr" hidden>${errs.phone ?? 'Please check the mobile number, it needs at least 8 digits'}</small>
    </label>
  </div>
  <div class="row2">
    <label for="qSuburb">Suburb <span class="req" aria-hidden="true">*</span>
      <input id="qSuburb" name="suburb" type="text" autocomplete="address-level2" required aria-describedby="qSuburbErr">
      <small class="field-err" id="qSuburbErr" hidden>${errs.suburb ?? 'Please add your suburb'}</small>
    </label>
    <label for="qEmail">Email <small>(optional)</small>
      <input id="qEmail" name="email" type="email" autocomplete="email">
    </label>
  </div>
  ${when(cfg.calculator?.formQuantityLabel, (label) => html`<label for="qQty">${label} <small>(optional)</small>
  ...
  <div class="alt"><span>${f.privacyNote ?? 'We will only use your details to reply to this enquiry.'}</span></div>
</form>`;
}
