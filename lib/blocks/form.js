 ...
 */
import { html, raw, when, each, present, telHref, esc } from '../render.js';
...
export function enquiryForm(cfg, classes = FORM_CLASSES.quote, { defaultService, defaultSuburb } = {}) {
  const f = cfg.form ?? {};
  const errs = f.errors ?? {};
  // Commercial services included too, not just cfg.services — a commercial
  // service page's form used to offer no matching option at all.
  const jobTypes = [...(cfg.services ?? []), ...(cfg.servicesCommercial ?? [])].map((s) => s.title).filter(present);
...
<form class="${c.form}" id="quoteForm" novalidate data-reveal>
  <div class="${c.row}">
    <label for="qService">${f.serviceLabel ?? 'What do you need?'}
      <select id="qService" name="service">
        ${each(options, (o) => html`<option${raw(o === defaultService ? ' selected' : '')}>${o}</option>`)}
      </select>
    </label>
  ...
  <div class="${c.row}">
    <label for="qSuburb"><span>Suburb${star}</span>
      <input id="qSuburb" name="suburb" type="text" autocomplete="address-level2" required aria-describedby="qSuburbErr"${raw(defaultSuburb ? ` value="${esc(defaultSuburb)}"` : '')}>
...
<div class="thanks" id="thanks" hidden role="status">
  ...
  ${when(phone.display, (d) => html`<p>In a hurry? Call <a href="${telHref(phone)}">${d}</a>.</p>`)}
