...
export function jsonLd(cfg) {
  ...
  // Suburb names only, unless a group names its own state. Tagging every
  // suburb with the business's region put Queanbeyan, Yass and Crookwell in
  // the ACT on Utopian's page; they are in NSW, over the border.
  const places = (cfg.serviceAreas ?? []).flatMap((g) =>
    (g.suburbs ?? []).map((s) => (g.state ? `${s}, ${g.state}` : s)));
  if (places.length) {
    node.areaServed = places.map((name) => ({ '@type': 'Place', name }));
  }

  // The owner is not the founder unless their own site says so. Utopian's
  // page told search engines Derek founded the business, which nothing they
  // publish states. `business.founder: true` once it is in `_source`.
  if (b.founder && b.ownerName) {
