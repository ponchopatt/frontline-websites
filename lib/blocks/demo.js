...
<div class="fl-demobar" id="demobar">
  <div class="wrap fl-demobar-in">
    <p>${d.bannerPrefix ?? 'Prepared for'} <b>${owner}</b> ${d.bannerSuffix ?? 'by Frontline Systems'}${
      when(site, (s) => html`<span class="fl-demobar-more"> — a preview of what ${s} could look like</span>`)}.</p>
...
export function demoFooterNote(cfg) {
  ...
  const credits = sampleCredits(cfg);
  const note = [text, credits.length ? `Sample photos: ${credits.join('; ')}.` : ''].filter(Boolean).join(' ');
  return when(note, (t) => html`<p class="fl-demo-note">${t}</p>`);
}

/*
 * CC BY and CC BY-SA photos have to be credited where they're shown. The
 * credit sat in config.json and never reached the page (Meyers, S&I, First
 * Point). CC0, Unsplash and Pexels ask for nothing, so they're left out.
 */
function sampleCredits(cfg) {
  const out = new Set();
  const walk = (v) => {
    if (Array.isArray(v)) return v.forEach(walk);
    if (!v || typeof v !== 'object') return;
    if (v.src && v.sample && /\bCC BY\b/i.test(v.license ?? '')) {
      // Credits are written as notes ("Sample photo: … (cropped to portrait)");
      // the page only wants who and which licence.
      const who = String(v.credit ?? '').split(/\.\s/)[0]
        .replace(/^sample photo:\s*/i, '').replace(/\s*\([^)]*\)\s*$/, '').replace(/\.$/, '').trim();
      const lic = String(v.license).match(/CC BY(?:-SA)?(?: \d\.\d)?/i)?.[0] ?? v.license;
      if (who) out.add(/\bCC BY\b/i.test(who) ? who : `${who} (${lic})`);
    }
    for (const [k, x] of Object.entries(v)) if (k !== 'sample') walk(x);
  };
  walk(cfg.photos);
  walk(cfg.services);
  return [...out];
