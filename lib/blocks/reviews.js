...
export function reviews(cfg, { title = 'What customers say', intro, className = '' } = {}) {
...
<section class="fl-reviews${raw(className ? ' ' + className : '')}" id="reviews">
  <div class="wrap">
    <div class="fl-reviews-head" data-reveal>
      <div class="head">
        <h2>${title}</h2>
        ${when(intro, (i) => html`<p class="lede">${i}</p>`)}
      </div>
  ...
  <div class="fl-reviews-rows" data-reveal>
    ${each(rows, (row, i) => html`<div class="fl-rv-row${raw(i === 1 ? ' rev' : '')}">
      ${each(row, (r) => card(r, false))}${each(row, (r) => card(r, true))}
    </div>`)}
  </div>
</section>`;
}
