...
export function reviews(cfg, {
  ...
  // Fewer than five can't fill a belt: the loop copy put the same card on
  // screen twice at once (CJG with one review; ACT with three at 1440px —
  // four cards are narrower than a 1920 screen too). Show them still, as a
  // grid from three up.
  const still = list.length < 5;
  const grid = still && list.length >= 3;
...
<section class="fl-reviews${raw(className ? ' ' + className : '')}" id="reviews">
  ...
  <div class="fl-reviews-rows" data-reveal>
    ${each(rows, (row, i) => html`<div class="fl-rv-row${raw(i === 1 ? ' rev' : '')}${raw(still ? ' still' : '')}${raw(grid ? ' grid' : '')}">
