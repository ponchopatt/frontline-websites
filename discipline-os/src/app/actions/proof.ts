...
import type { ActionResult, ProofItem } from "@/lib/types";
...
export async function uploadProof(form: FormData): Promise<ActionResult<ProofItem>> {
  ...
  const parsed = metaSchema.safeParse({ date: text("date"), taskId: text("taskId"), habitId: text("habitId"), label: text("label") });
  ...
  const { data, error } = await viewer.supabase
    .from("proof_uploads")
    .insert({ local_date: date, storage_path: path, task_id: taskId, habit_id: habitId, note: label })
  ...
  return ok({ id: data.id, url: signed?.signedUrl ?? null, taskId, habitId, label, uploadedAt: data.uploaded_at });
}
