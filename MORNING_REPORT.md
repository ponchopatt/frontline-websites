...
Finish round: **2 labelled sample photos** (hero: a pool at the steel stage; statement: a crew in a fresh dig; CC BY, credited in the footer). Their own small photos are now shown only at about native size. The single review is a still card. Before live, swap both samples for Craig's own. Earlier notes below still stand.
...
### Meyers Building Group — shipped (4.42 in review, then the fixes the reviewer said would lift it to 4.5)
Finish round: no AI-upscaled photos; their own 960px photos used only at native size (gallery and two service cards); **4 labelled sample photos** (hero cottage, statement, porch, wall frames; CC BY/BY-SA, credited in the footer). No reviews section. Before live, swap every sample for Darren's own. Earlier notes below still stand.
